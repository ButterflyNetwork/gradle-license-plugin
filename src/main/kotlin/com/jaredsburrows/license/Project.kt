...
private fun Project.buildPomInput(
  ...
  if (explicitDependencies != null) {
    // Parse allowlist into group:module pairs (ignore version for matching)
    val validDeps =
    ...
    val filteredRoots =
      roots
        .filter { coordinate ->
          ...
          if (parts.size >= 2) {
            allowlist.contains("${parts[0]}:${parts[1]}")
          } else {
            false
          }
        }.toMutableSet()

    // Find which allowlist entries have no matching root
    val resolvedKeys =
      filteredRoots
        .map { coordinate ->
          val parts = coordinate.split(":")
    ...
    val missingKeys = allowlist - resolvedKeys
    ...
    if (missingKeys.isNotEmpty()) {
      ...
      for (key in stillMissing) {
        ...
        // Try individual resolution using version from broad scan or explicit list
